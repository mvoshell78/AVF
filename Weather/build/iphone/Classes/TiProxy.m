...
//Rare exceptions to indicate a bug in the _voshell_michael_week1_project code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_voshell_michael_week1_project = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_voshell_michael_week1_project]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.voshell_michael_week1_project.module"
