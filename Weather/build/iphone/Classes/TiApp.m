...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kVoshell_Michael_week1_projectUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kVoshell_Michael_week1_projectUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Voshell_Michael_week1_project app using the node.js CLI
