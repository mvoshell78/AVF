...
NSString * Voshell_Michael_week1_project$ModuleRequireFormat = @"(function(exports){"
...
@implementation Voshell_Michael_week1_projectObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_voshell_michael_week1_project);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_voshell_michael_week1_project gc];
}
...
{
	// create Voshell_Michael_week1_project global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Voshell_Michael_week1_project" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_voshell_michael_week1_project = [[Voshell_Michael_week1_projectObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_voshell_michael_week1_project];
	
	NSString *_voshell_michael_week1_projectNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _voshell_michael_week1_projectNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_voshell_michael_week1_project valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Voshell_Michael_week1_project$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.voshell_michael_week1_project.kroll" 
