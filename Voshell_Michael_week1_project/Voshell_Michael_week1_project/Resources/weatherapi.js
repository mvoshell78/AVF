...
// returns dimensions of device to make dynamic views

var createUrl = function(url){
	console.log("function", url);
	xhr.open("GET", url);
	xhr.send();// required 
	//takes concat url from geo.js and passes it into the xhr function below 
	
};

exports.createUrl = createUrl;
// exports createUrl function so that geo.js can pass to it

var remoteResponce = function(){
	var json = JSON.parse(this.responseText);
	// gets data from api formats it as json
	
	// var for data returned from the api
...
var xhr = Ti.Network.createHTTPClient({
	onload :  remoteResponce, // what to do onloading the api
	onError: remoteError, // what to do on error
	timeout : 5000// times out at 5 seconds
