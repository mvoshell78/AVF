...
//Rare exceptions to indicate a bug in the _taco code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_taco = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_taco]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.taco.module"
